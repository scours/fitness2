%fileid_20191113
%INRA\Olivier Vitrac - 19/08/2018, 28/08/2019, 03/09/2019 (major release, TOC), 07/09/2019 (dependencies), 07/12/2019 (svg)
%
...
if ~exist('cPPTx','var') && ~forced
    ...
    simulation = true;
    forced = true;
    fileid_20191207               %<----------------- update it when needed (/* ATTENTION */) (/* ATTENTION */) (/* ATTENTION */)
...
for i=1:nPPTx
    ...
    if ~PPTx(i).ok
        ...
        if ~simulation
            ...
            if forcedupdate
                pptx2reveal_v2(pptx2reveal_setup,'TOC',TOC,'static','repair','forcedupdate')
            else
                pptx2reveal_v2(pptx2reveal_setup,'TOC',TOC,'static','repair')
            end
        end
    end
end
