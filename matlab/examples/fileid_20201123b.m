...
menulib = struct( ...
    'source',cellfun(@(f) fullfile(sourcemenu,f),          {'content_transitional.json'      'css/menu.css' 'css/sideBarMenu.css' 'css/slidesSearch.css' 'js/menu.js' 'js/jquery.min.js' 'js/jsjsonsearch2de.js' 'js/searchCore.js' 'js/sideBarMenu.js' },'UniformOutput',false),...
    'destination',cellfun(@(f) fullfile(destinationmenu,f),{'json/content_transitional.json' 'css/menu.css' 'css/sideBarMenu.css' 'css/slidesSearch.css' 'js/menu.js' 'js/jquery.min.js' 'js/jsjsonsearch2de.js' 'js/searchCore.js' 'js/sideBarMenu.js' },'UniformOutput',false) ...
);


%% Check whether a global menu is defined
menukey = struct('head','menucontentHEAD','body','menucontentBODY'); % name of the keys in the ODS file
menu = struct('head',struct('tag','</head>','content','','where','before','mode','insert','endtag',''),...
              'body',struct('tag','<body>','content','','where','after','mode','substitute','endtag','<div class="reveal">'));
menuinsert = struct([]);
menusubstitute = struct([]);
