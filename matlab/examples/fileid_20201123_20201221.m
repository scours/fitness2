...
% $ revision 2020/02/06 $
% $ revision 2020/11/23 $ - modified with Steward Ouadi
% $ revision 2020/12/01 $ - fix part using the same strategy as used in update_content_JSON
% $ revision 2020/12/02 $ - several fixes to accomodate reveal-md 4.20 and to fix part (based on )
% $ revision 2020/12/14 $ - implement the new menu from Steward Ouadi
% $ revision 2020/12/16 $ - reveal-md 4.3 (use  sudo yarn global upgrade)
% $ revision 2020/12/21 $ - major update with videos and new content
% $ revision 2020/12/22 $ - add new insertion for new left menu (version 0.33)

% ===============================================================
%   source is a manual update of 20201123 on LX-Olivier2020
%   destination is fresh for the last publication of FITNess
% ===============================================================
...
usr.set({ 'local','current','fileid'},...
        {'/home/@username/Bulk/fitness2020/sandbox',...
         'fileid_20201123_20201221.ods',...
        '$local/$current'},'path');
fileid = usr.fileid; %fileid = '/home/olivi/Bulk/fitness2019/sandbox/fileid_20190814.ods';
def = alias; %alias('protectpath',false);
def.load(fileid)
% standard behavior: forcedupdate = false; forcecleandepencies = true;
forcedupdate = false;       % <--- set it to true ONLY to update PREFETCH files (only for full update)
forcecleandepencies = true; % <--- true is required to update dependencies (recommended)
...
menulib = struct( ...
    'source',cellfun(@(f) fullfile(sourcemenu,f),          {'content_transitional.json'      'css/menu.css' 'css/sideBarMenu.css' 'css/slidesSearch.css' 'js/menu.js' 'js/jquery.min.js' 'js/jsjsonsearch2de.js' 'js/searchCore.js' 'js/sideBarMenu.js', 'js/utils.js' 'js/content_transitional.js'},'UniformOutput',false),...
    'destination',cellfun(@(f) fullfile(destinationmenu,f),{'json/content_transitional.json' 'css/menu.css' 'css/sideBarMenu.css' 'css/slidesSearch.css' 'js/menu.js' 'js/jquery.min.js' 'js/jsjsonsearch2de.js' 'js/searchCore.js' 'js/sideBarMenu.js', 'js/utils.js' 'js/content_transitional.js'},'UniformOutput',false) ...
);


%% Check whether a global menu is defined
menukey = struct('head' ,'menucontentHEAD',...
                 'body1','menucontentBODYBEGIN',...
                 'body2','menucontentBODYEND'); %   name of the keys in the ODS file, menukey is local nomenclature
menu = struct(...
    'head' ,struct('tag','</head>','content','','where','before','mode','insert','endtag',''),...
    'body1',struct('tag','<body>' ,'content','','where','after','mode','substitute','endtag','<div class="reveal">'),...
    'body2',struct('tag','</body>','content','','where','before','mode','insert','endtag','') ...
          );
menuinsert = struct([]);        % object used by pptx2reval_v2
menusubstitute = struct([]);    % object used by pptx2reval_v2
for m=fieldnames(menukey)'
    ...
    % Determine wether the menu should be introduced using the insertion or replacement method
    switch menu.(m{1}).mode
        case 'insert' % default behavior (which is called replace in pptx2reveal_v2)
            menuinsert(1).(m{1}) = menu.(m{1});
        case 'substitute' % new behavior
            menusubstitute(1).(m{1}) = menu.(m{1});
        otherwise
            error('unknwon menu method ''%s''',menu.(m{1}).mode)
    end
end
...
if ~exist(def.sourcePPTfolder,'dir'), error('the source folder ''%s'' does not exist',def.sourcePPTfolder), end
...
%% classification // self added on 2020-12-01
% ===== self robust parameter extraction from update_content_JSON (version 2020-11-30)
...
for i=1:nPPTx
    % add part if missing
    if isempty(regexp(PPTx(i).file,'part\s*\d','match','ignorecase')), suffix = '_part1.pptx'; else, suffix = '.pptx'; end
    PPTx(i).file2 = [PPTx(i).file suffix];
    ...
    tmp = sectionextractor(PPTx(i));
    % robust extraction of [level section unit part] numbers
    PPTx(i).self = [find(ismember(listlevels,tmp.level)) str2double(tmp.section) str2double(tmp.unit) str2double(tmp.part)];
end
% final sort with self
[~,ind] = sortrows(cat(1,PPTx.self));
levels = levels(ind,:);
PPTx = PPTx(ind);
% text extraction
...
for i=1:nPPTx
    ...
    PPTx(i).p = PPTx(i).self(4); %PPTx(i).ipart;
...
for i=1:nPPTx
    ...
    PPTx(i).INPUTS.INPUT6 = sprintf('part%d',PPTx(i).p); %sprintf('part%d',PPTx(i).ipart)
    ...
    if ~PPTx(i).ok
        ...
        if ~simulation
            ...
            if forcedupdate
                pptx2reveal_v2(pptx2reveal_setup,'TOC',TOC,'replace',menuinsert,'substitute',menusubstitute,'static','repair','forcedupdate') %#ok<UNRCH>
            else
                pptx2reveal_v2(pptx2reveal_setup,'TOC',TOC,'replace',menuinsert,'substitute',menusubstitute,'static','repair')
            end
        end
    end
end
...
%% copy menu file libraries (CSS, JS, JSON)
for i=1:length(menulib)
    [ps,fs,es] = fileparts(menulib(i).source);
    pd = fileparts(menulib(i).destination);
    if ~exist(ps,'dir'), error('the source folder ''%s'' does not exist for the menu file ''%s%s''',ps,fs,es), end
    if ~exist(menulib(i).source,'file'), error('the source file ''%s%s'' does not exist in ''%s''',fs,es,ps), end
    if ~exist(pd,'dir'), dispf('>> creation of the folder ''%s'' in ''%s''',lastdir(pd),rootdir(pd)), mkdir(pd); end
