...
function paginationButtonOnClickHandler(event, showSlides) {
  ...
  if (showSlides === true) {
    ...
    // Add topics listeners here, so on page change, listeners can be added on topics displayed on the current page
    addTopicsListeners();
  }
...
async function extractOnlyNeededInfo() {
  let flatcontent = await getDatabaseData(
    '../src/json/content_transitional.json'
  );
